...
contract Port3Layer2Staking is Context, Initializable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 apy;                    // reward apy for pool, 1000000 for 100%
    ...
    function setPoolReward(uint256 _pid, uint256 _apy, bool _isOpenReward, bool _isRewardEth, uint256 _startTime, uint256 _endTime) external onlyOwner {
    ...
    function pendingTokenReward(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 remain = 0;
        uint256 stakeTime = getTimeCount(_pid, user.lastHarvestTime, block.timestamp);
        uint256 userReward = user.amount.mul(pool.apy).mul(stakeTime).div(365).div(86400).div(1e6);
